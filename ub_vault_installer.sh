#!/bin/bash
# cromaca file server installer
# cmkfs
# For ubuntu server 14.04 (headless)
#
# NOTE: do not run this as $ sudo sh <script> ...
#       as this invokes dash?? and has no <<< redirect
#       run as $ sudo <script> ...
#
#	Pull this from git onto a vanilla ubuntu server and run as sudo
#	$ sudo ub_vault_installer.sh <user> <password> <database> <host>
#
#	
#	
###############################################################################
...
GK_DOMAIN=${GK_DOMAIN:-cmkfs.virtnet}
...
DB_SCRIPT=$DIR/'lib/create-bare-db.sql'
...
##############################################################################
cp $DIR/lib/gatekeeper.sh $GK_HOME/app/bin/
...
# if there exists a database already you can populate the variable for use
mysql -h "$DB_HOST" -u "$DB_USER" -p "$DB_ROOT_PW" "$DB_NAME" < $DB_SCRIPT 
